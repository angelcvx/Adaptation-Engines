...
public class externalClassLoader {
    ...
    public static Class<?> loadClass (File dexOutputDir, String className, String classesDirectory, String fileName) {
        ...
        if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            Log.v("ExternalLoader", "LoadDexClasses is only available for ICS or up");
        } else {

            File[] files = new File(classesDirectory).listFiles();

            if (files == null) {
                Log.d("ExternalLoader", "There was no classes in" + classesDirectory);
            } else {
                ...
                for (File file : files) {
                    if (file.getName().equals(fileName)) {
                        ...
                        try {
                            ...
                            Log.d("ExternalLoader", e.getMessage());
                        }
                    }
                }
            }
