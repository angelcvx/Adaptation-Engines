...
public class LoadLeaguesAdaptation implements LoadInterface{
    ...
    public boolean loadLeagues(){
        ...
        if (files == null) {
            ...
            for (File file : files) {
                if (usedLeagues.contains(positionOf(file.getName())+OFFSET)) {
                    read(file);
                }
            }
        }
    ...
    public boolean loadLeague(int pos){
        ...
        directory.mkdir();
        File[] files = directory.listFiles();
        String leagueToLoad = allLeagues[pos - OFFSET];
        if (files == null) {
            return false;
        } else {
            for (File file : files) {
    ...
    public String getLeague (int pos){
        if (!leagues.containsKey(pos) && pos < allLeagues.length + OFFSET) {
            loadLeague(pos);
        }
    ...
    public void setUsedLeagues(List<Integer> usedLeagues) {
        this.usedLeagues = usedLeagues;
    }

    public static void setLeagues(Map<Integer, String> leagues) {
        LoadLeaguesAdaptation.leagues = leagues;
    }
